# master/003_update_task.ps1
Write-Host "[MASTER][003] Update Task Scheduler..."
...
if (-not (Test-Path $masterData)) { New-Item -Path $masterData -ItemType Directory -Force | Out-Null }

$versionFile = Join-Path $masterData "task_version.txt"
$currentVersion = if (Test-Path $versionFile) { (Get-Content $versionFile -ErrorAction SilentlyContinue) } else { "0" }

Write-Host "[MASTER][003] Current=$currentVersion Target=$taskVersion"
...
if ($currentVersion -ne $taskVersion) {
    ...
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger @($t1,$t2) -Principal $principal -Settings $settings `
        -Description "POS Auto Update Master v$taskVersion (D:\ChuChau\POS-Auto-Update)" -Force | Out-Null

    Set-Content -Path $versionFile -Value $taskVersion -Encoding UTF8
    Write-Host "[MASTER][003] Task updated to v$taskVersion" -ForegroundColor Green
} else {
    Write-Host "[MASTER][003] Task already up-to-date, skip." -ForegroundColor Yellow
}

Write-Host "[MASTER][003] DONE" -ForegroundColor Green
